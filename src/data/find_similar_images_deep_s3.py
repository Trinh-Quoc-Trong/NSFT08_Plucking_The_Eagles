...
def find_similar_images_deep(images_dir, min_similarity=0.97, max_similarity=0.98, num_samples_to_show=5):
    ...
    for filename in tqdm(image_files, desc="Trích xuất đặc trưng"):
        ...
        features = extract_features(
            image_path, feature_extractor, device, transform)
    ...
    print(
        f">>> Đang tìm kiếm các cặp tương đồng trong khoảng [{min_similarity}, {max_similarity})...")
    similar_pairs = []
    ...
    for file1, file2, similarity in random_pairs_to_show:
        ...
        fig.suptitle(
            f'Cặp tương đồng - Độ tương đồng Cosine: {similarity:.4f}', fontsize=16)
        plt.show()
